...
class MatchsServices {
  ...
  static async create(data: Match) {
    ...
    if(homeTeam ===  awayTeam) return  { code: StatusCode.UNAUTHENTICATED, response: { message: MessageError.equal } };
    ...
    if (!homeTeam || !awayTeam) {
      return  { code: StatusCode.UNAUTHENTICATED, response: { message: MessageError.teamNotFound } };
    }
